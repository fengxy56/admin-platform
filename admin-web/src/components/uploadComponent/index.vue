...
<template>
	<div>
		...
		<Upload v-show="show" :disabled='disabled' ref="upload" :show-upload-list="false" :default-file-list="defaultList" :on-success="handleSuccess"
		 ...
		 :before-upload="handleBeforeUpload" type="drag" :action="`${this.$api.uploadUrl}?projectId=${projectId}&typeId=${typeId}`"
		...
		<Modal title="图片预览" v-model="visible">
			<img :src="`${this.$api.getFileUrl}?fileType=img&imgType=0&fileId=${imgName}`" v-if="visible" style="width: 100%">
			<div slot="footer">
	...
	export default {
		...
		methods: {
			...
			handleSuccess(res, file ,fileList) {
				file.url =
					`${this.$api.getFileUrl}?fileId=${res.fileId}&fileType=img&imgType=1`
