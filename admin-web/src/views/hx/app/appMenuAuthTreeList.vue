...
export default {
  ...
  methods: {
    //获取权限菜单数据
    async getUserRoleAppMenuVOList() {
      this.loading = true;
      const param = { authType: this.authType, authId: this.authId, parentAppId: this.queryParams.appId  };
      const result = await this.$api.getUserRoleAppMenuVOList(param);
      if (result.code === 1) {
    ...
    async save() {
      if (this.selectAction) {
        ...
        const result = await this.$api.saveUserRoleHxAppMenuAuth(param);
        if (result.code === 1) {
