...
export default {
  ...
  methods: {
    //获取权限菜单数据
    async getTreeSysAuthMenuList() {
      this.loading = true;
      const result = await this.$api.getTreeSysAuthMenuList(this.queryParams);
      if (result.code === 1) {
  ...
  mounted() {
    ...
    this.queryParams.authType = this.authType;
    this.getTreeSysAuthMenuList();
  }
};
</script>